...
type Route =
  ...
  | { name: 'profile' }
  | { name: 'catalog' }
  | { name: 'tool'; toolId: string; query?: string };

export function App({ tools, bridge }: { tools: ToolEntry[]; bridge: TuiBridge }) {
  const { exit } = useApp();
  ...
  switch (route.name) {
    case 'projects':
      return (
        <Projects
          ...
          bridge={bridge}
          onOpen={s => setRoute({ name: 'tool', toolId: s.toolId, query: s.query })}
    ...
    case 'tool':
      return <ToolView toolId={route.toolId} query={route.query} bridge={bridge} onBack={() => setRoute({ name: 'gallery' })} />;
    default:
